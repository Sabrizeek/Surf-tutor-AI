...
import { PoseLandmarks, PoseLandmark } from './poseDetection';
...
export interface BodyMetrics {
  ...
  shoulderRotation: number; // Degrees of rotation
  bodyLean: {
    forward: number; // Positive = leaning forward
    lateral: number; // Positive = leaning right
  };
  centerOfMass: {
    x: number;
    y: number;
 ...
 */
export function calculateBodyMetrics(landmarks: PoseLandmarks): BodyMetrics | null {
  if (!landmarks) {
    return null;
  }
 ...
 */
export function getAngleDifference(currentAngle: number, targetAngle: number): number {
