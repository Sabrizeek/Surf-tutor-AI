...
const smoothingBuffers: Map<string, SmoothingBuffer> = new Map();

/**
 * Initialize smoothing buffer for a landmark key
 */
function initBuffer(key: string, maxSize: number = 5): SmoothingBuffer {
...
function smoothLandmark(
  ...
  alpha: number = 0.4 // Smoothing factor (0.0 = no smoothing, 1.0 = full smoothing)
): PoseLandmark | null {
  if (!current) {
    return null;
  }
...
export function smoothLandmarks(
  ...
  const keys: (keyof PoseLandmarks)[] = [
    'nose',
    'leftEye',
    'rightEye',
    'leftEar',
 ...
 */
export function clearSmoothingBuffers(): void {
