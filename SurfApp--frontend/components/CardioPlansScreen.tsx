...
const AnimatedPlanCard = ({ plan, index, onStartWorkout, onExplain }: any) => {
  ...
  const exercises = typeof plan.exercises === 'string' 
    ? plan.exercises.split(';').map((e: string) => e.trim())
    ...
    >
      ...
      {exercises.length > 0 && (
        <View style={styles.exercisesPreview}>
          ...
          {exercises.slice(0, 4).map((exercise: string, exIndex: number) => (
