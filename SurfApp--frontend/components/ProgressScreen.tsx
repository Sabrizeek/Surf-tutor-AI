...
function calculatePoseStats(progressData: any): PoseStats {
  ...
  const scores: { [drillId: string]: number[] } = {};
...
function calculateARStats(progressData: any): ARStats {
...
export default function ProgressScreen() {
  ...
  const [poseEarnedBadges, setPoseEarnedBadges] = useState<string[]>([]);
  const [arEarnedBadges, setArEarnedBadges] = useState<string[]>([]);
  const [badgeProgress, setBadgeProgress] = useState<BadgeProgress>({});
  const [poseProgress, setPoseProgress] = useState<any>(null);
  const [arProgress, setArProgress] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [activeTab, setActiveTab] = useState<'stats' | 'badges' | 'history' | 'pose' | 'ar'>('stats');
  ...
  const calculatePoseAccuracy = (): number => {
