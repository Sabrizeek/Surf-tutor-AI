...
// Native pose detection - fallback to mock for now
// TODO: Replace with actual native plugin when available
// @ts-ignore
import Icon from 'react-native-vector-icons/MaterialIcons';
import { analyzePose, PoseLandmarks, PoseLandmark, isPersonDetectedProgressive } from '../utils/poseDetection';
import { smoothLandmarks, clearSmoothingBuffers } from '../utils/landmarkSmoothing';
import { 
  calculateStabilityScore, 
  convertNativePoseToLandmarks, 
  DEFAULT_POSE_OPTIONS 
} from '../utils/nativePoseDetection';
import { speakGuidance, resetVoiceGuidance, stopSpeaking } from '../utils/voiceGuidance';
import { calculateBodyMetrics } from '../utils/kinematics';
import { evaluateDrillRules } from '../utils/drillRulesEngine';
import AngleArcOverlay from './AngleArcOverlay';
import ReferenceSkeletonOverlay from './ReferenceSkeletonOverlay';
import LiveScoreBar from './LiveScoreBar';

// Animated component wrappers with proper typing
const AnimatedView = Animated.View as ComponentType<Animated.AnimatedProps<ViewProps>>;
const AnimatedText = Animated.Text as ComponentType<Animated.AnimatedProps<TextProps>>;
...
interface DynamicGuideRectangleProps {
  personBoundingBox: {
    x: number;
    y: number;
    width: number;
  ...
  calibrationStatus: 'not_detected' | 'detecting' | 'positioning' | 'too_close' | 'too_far' | 'off_center' | 'ready';
...
export default function PosePracticeScreen() {
  ...
  const [drillTimer, setDrillTimer] = useState(0); // seconds
  ...
  const cameraReadyRef = useRef(false); // Track if camera is ready for photo capture
  const poseDetectionIntervalRef = useRef<NodeJS.Timeout | null>(null); // Track pose detection interval
  const [useRealPoseDetection, setUseRealPoseDetection] = useState(true); // Toggle for real vs mock
  const [isProcessingPose, setIsProcessingPose] = useState(false); // UI loading state
  const [connectionStatus, setConnectionStatus] = useState<'connected' | 'disconnected' | 'error'>('connected');
  const [currentLandmarks, setCurrentLandmarks] = useState<PoseLandmarks | null>(null); // For skeleton visualization
  const [showSkeleton, setShowSkeleton] = useState(true); // Toggle skeleton overlay
  const [detectionConfidence, setDetectionConfidence] = useState(0); // 0-100
  const [detectionStage, setDetectionStage] = useState<'none' | 'preview' | 'partial' | 'full'>('none');
  const [currentBodyMetrics, setCurrentBodyMetrics] = useState<ReturnType<typeof calculateBodyMetrics> | null>(null); // Phase 2.3: For angle arcs
  const [showReferenceSkeleton, setShowReferenceSkeleton] = useState(true); // Phase 3.1: Toggle reference skeleton
  ...
  const scoreHistoryRef = useRef<number[]>([]); // Last 30 scores for smoothing
  const feedbackTimeoutRef = useRef<NodeJS.Timeout | null>(null); // For debouncing feedback
  const [averageScore, setAverageScore] = useState(0);
  const [scoreTrend, setScoreTrend] = useState<'up' | 'down' | 'stable'>('stable');
  
  // Phase 4.3: Session replay - store landmark history
  const landmarksHistoryRef = useRef<Array<{
    timestamp: number;
  ...
  const LANDMARK_SAVE_INTERVAL = 100; // Save every 100ms (10 FPS)
  
  // Phase 3.3: Shared state for immediate stop
  const isStoppingRef = useRef(false); // Shared flag for immediate stop
  
  // Phase 1.2: Dynamic visual guidance state
  const [personBoundingBox, setPersonBoundingBox] = useState<{
    x: number;
    y: number;
    width: number;
    height: number;
  ...
  const [calibrationStatus, setCalibrationStatus] = useState<'not_detected' | 'detecting' | 'positioning' | 'too_close' | 'too_far' | 'off_center' | 'ready'>('not_detected');
  ...
  const [bodyCompleteness, setBodyCompleteness] = useState<{
    ...
    legs: boolean;
    feet: boolean;
  }>({ head: false, torso: false, legs: false, feet: false });
  ...
  const processPoseFromFrame = async (base64Image: string): Promise<void> => {
    if (!selectedDrill) {
      return;
    }
    ...
    try {
      ...
      try {
        ...
        const { poseAPI } = require('../services/api');
        ...
        // Fallback to mock
        const { generateMockLandmarks, adjustLandmarksForDrill, addVariation } = require('../utils/mockPoseDetector');
      ...
      if (isReadyForAction) {
        ...
        if (ruleResults.length > 0) {
          ...
          const ruleFeedback = ruleResults
            ...
            .map(r => ({
              text: r.message,
              type: r.severity === 'error' ? 'error' : r.severity === 'warning' ? 'warning' : 'info' as 'error' | 'warning' | 'info',
        ...
        if (confirmedPersonDetected && analysis.personDetected) {
          const newFeedback: FeedbackItem[] = analysis.feedback.map((text) => {
          ...
          // Update stats
          if (selectedDrill) {
            setDrillStats((prev: Record<string, DrillStats>) => {
  ...
  const handleStopPractice = async () => {
    ...
    if (selectedDrill) {
      ...
      try {
        const { progressAPI } = require('../services/api');
        const { checkPoseBadges, awardBadge } = require('../utils/badgeSystem');
        
        // Phase 4.1: Calculate XP and prepare categorized progress data
        const { calculatePoseXP, calculateXPProgression } = require('../utils/xpSystem');
        ...
        const fullProgress: any = userProgress.pose || userProgress.poseEstimation || {};
  ...
  const renderSkeleton = (landmarks: PoseLandmarks, stage: 'none' | 'preview' | 'partial' | 'full') => {
    ...
    const skeletonLines: (React.ReactElement | null)[] = [];
    const skeletonJoints: (React.ReactElement | null)[] = [];

    // Helper to create a line
    const createLine = (p1: PoseLandmark | undefined, p2: PoseLandmark | undefined, key: string) => {
    ...
    const createJoint = (p: PoseLandmark | undefined, key: string) => {
      if (!p) return null;
      return (
        <View
          key={key}
            ...
            <>
              ...
              <View style={styles.overlayTop}>
                ...
                {personDetected && feedback.length > 0 && (
                  ...
                  >
                    {feedback
                      .sort((a: FeedbackItem, b: FeedbackItem) => b.priority - a.priority)
                      .slice(0, 3)
